...
export class TicketController {
  ...
  @Post('/')
  async create(@Body() dto: TicketDTO, @Headers('unique-trace-id') uniqueTraceId: string, @Headers('firstName') firstName: string, @Headers('lastName') lastName: string, @Headers('email') email: string): Promise<TicketResponse> {
    try {
      const result: Ticket = await this.service.create(dto, lastName, firstName, email);
  ...
  @Post('/bulk')
  async bulkCreate(@Body() dto: TicketsDTO, @Headers('unique-trace-id') uniqueTraceId: string, @Headers('firstName') firstName: string, @Headers('lastName') lastName: string, @Headers('email') email: string): Promise<TicketResponse> {
    try {
      const result: Array<Ticket> = await this.service.bulkCreate(dto.getTickets(), lastName, firstName, email);
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'The Tickets have been successfully updated.'
  })
  @ApiHeader({
    name: 'unique-trace-id',
    description: 'Autogenerated trace id',
  ...
  @ApiHeader({
    name: 'user_role',
    description: 'User role allowed to update Tickets ',
    required: true
  })
  @ApiHeader({
    name: 'Authorization',
  ...
  @Patch('/bulk')
  async bulkUpdate(@Body() dto: UpdateTicketsDTO, @Headers('unique-trace-id') uniqueTraceId: string) {
    try {
      await this.service.bulkUpdate(dto.getTickets());
    } catch (error) {
      this.logger.log({
        level: 'error',
        message: 'An Error occurred while trying to Bulk Update Tickets ',
        method: this.bulkUpdate.name,
        err: error,
        'unique-trace-id': uniqueTraceId
      });
      this.utilsService.throwInternalServerIfErrorIsNotHttpExcetion(error);
